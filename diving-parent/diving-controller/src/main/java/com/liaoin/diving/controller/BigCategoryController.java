...
public class BigCategoryController  {
    ...
    @PostMapping("/insert")
    @ApiOperation("新增")
    public Result save(@RequestBody BigCategory bigCategory) {
    ...
    @PostMapping("/update")
    @ApiOperation("修改")
    public Result update(@RequestBody BigCategory bigCategory) {
    ...
    public Result findOne(@RequestParam Integer id) {
        BigCategory bigCategory = bigCategoryService.findOne(id);
        if (bigCategory == null) {
            return new Result(300, "暂无数据", null);
        }
        return new Result(200, "查询成功", bigCategory);
    }
    ...
    public Result pageQuery(@PageableDefault Pageable pageable, @RequestBody BigCategory bigCategory) {
        PageBean<BigCategory> pageBean = bigCategoryService.pageQuery(pageable, bigCategory);
        if (pageBean == null) {
            return new Result(300, "暂无数据", null);
        }
